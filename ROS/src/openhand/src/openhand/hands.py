...
class OpenHand():
	...
	def __init__(self,port,servo_ids,series="RX"):
		self.port = port
		if DEBUG > 0:
			print "opening the USB to Dynamixel port"
		...
		for servo_id in self.servo_ids:
			...
			if DEBUG > 0:
				print ("Initializing servo number %d" % servo_id)
	...
	def moveMotor(self,index,amnt):
		if amnt < 0. or amnt > 5.0:
			print "[WARNING] motion out of bounds, capping to [0,5.0]. Index: "+repr(index)+", Cmd:"+repr(amnt)
		amnt = min(max(amnt,0.),5.0)
...
#		val = min(1.0,max(val,0))	#by design, can exceed default max torque value
		print val
		self.servos[0].enable_torque_mode()
		self.servos[0].apply_torque(val)
		time.sleep(self.pause*2)
